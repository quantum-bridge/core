...
type Bridge interface {
	...
	CheckFungible(txHash string, eventIndex int, tokenChain datashared.TokenChain) (*datashared.FungibleLock, error)
	// CheckNonFungible is a method that checks the status of the given non-fungible token transfer.
	CheckNonFungible(txHash string, eventIndex int, tokenChain datashared.TokenChain) (*datashared.NonFungibleLock, error)
	...
	WithdrawFungible(tokenChain datashared.TokenChain, from, to, txHash string, eventIndex int, amount *big.Int, RawTxData *[]byte) (interface{}, error)
	// WithdrawNonFungible is a method that withdraws the given non-fungible token from the bridge contract.
	WithdrawNonFungible(tokenChain datashared.TokenChain, from, to, txHash string, eventIndex int, tokenID, tokenURI string, RawTxData *[]byte) (interface{}, error)
